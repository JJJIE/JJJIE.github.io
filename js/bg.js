var RENDERER = {
	...
	setParameters : function(strategy){
		...
		this.$container = $('#container_particle-container');
...
var STRATEGY = {
	...
	createBigSphere : function(){
		var cosTheta = Math.random() * 2 - 1,
			sinTheta = Math.sqrt(1 - cosTheta * cosTheta),
			phi = Math.random() * 2 * Math.PI;
			
		return {
			x : this.SCATTER_RADIUS * sinTheta * Math.cos(phi) * 2,
			y : this.SCATTER_RADIUS * sinTheta * Math.sin(phi) * 2,
			z : this.SCATTER_RADIUS * cosTheta * 2,
			hue : Math.round(phi / Math.PI * 80)
		};
	},
